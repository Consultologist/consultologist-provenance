...
4. Recompute `workflowOutputHash` from `assembledDocuments` by the definition `workflowOutputHashVersion` names, and each `documentHash` from its `text`. They must match what is served.
