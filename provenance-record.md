...
| `nodeOutputs` | snapshot | Per node instance (`nodeId` or `nodeId:itemId`): `{ nodeId, label, status, inputHash?, outputHash?, completedAtUtc?, error? }`. The two hashes are the per-node hashes (hash-definitions.md § 4). |
...
1. Resolve `workflowPackage` and `catalogRef` through their registries. You now hold the package that ran and the catalog that mapped its contracts to agents.
